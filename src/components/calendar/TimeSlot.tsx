...
interface TimeSlotProps {
  ...
  day: number;
  roomId: string;
  onDrop: (routine: any, timeSlot: { hour: number; minute: number; day: number; roomId: string }) => void;
  onMoveRoutine?: (routine: any, timeSlot: { hour: number; minute: number; day: number; roomId: string }) => void;
  ...
  const [{ isOver, canDrop }, drop] = useDrop({
    ...
    drop: (item: any, monitor) => {
      ...
      if (item.type === 'scheduled-routine' && onMoveRoutine) {
        ...
        console.log('Moving scheduled routine:', item.routine.songTitle, 'to', timeSlot);
        onMoveRoutine(item.routine, timeSlot);
